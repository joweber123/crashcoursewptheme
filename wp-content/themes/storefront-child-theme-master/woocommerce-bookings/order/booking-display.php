	...
	foreach ( $booking_ids as $booking_id ) {
		...
		<div class="wc-booking-summary">
			<strong class="wc-booking-summary-number">
				...
				printf( __( 'Travel Session #%s', 'woocommerce-bookings' ), esc_html( $booking->get_id() ) );
				?>
			...
			<div class="wc-booking-summary-actions">
                <?php if (! is_account_page()) : ?>
                    <?php if ( $booking_id && function_exists( 'wc_get_endpoint_url' ) && wc_get_page_id( 'myaccount' ) && 0 !== $booking->get_customer_id() ) : ?>
                        <a href="<?php echo esc_url( wc_get_endpoint_url( $endpoint, '', wc_get_page_permalink( 'myaccount' ) ) ); ?>"><?php _e( 'View my Travel Session &rarr;', 'woocommerce-bookings' ); ?></a>
                    <?php endif; ?>
                <?php endif; ?>
			</div>
		</div>
